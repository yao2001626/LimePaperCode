/* Compile under Linux with "cc big.simp.c -o big.simp -lpthread"; run with
"./big.simp". Does not run under macOS as unnamed semaphores (using sem_init)
are not supported.
...
This is a simplified version: the body of pong(Worker *this, int this_id),
...
is replaced by:
  pong(w, id); // open monitor call
As the body does not modify the state of this, the simplification is safe,
however, it still changes the behaviour as a worker could be busy pinging or
being pinged which receiving a pong. The simplified version is included as it
illustrates the overhead of successive calls to sem_wait() and sem_post().
*/
#include <pthread.h>
#include <stdio.h>
#include <assert.h>
